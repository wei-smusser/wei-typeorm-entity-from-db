...
async function useInquirer(options: options): Promise<options> {
    ...
    if (customizeGeneration) {
            ...
            await inquirer.prompt([
                {
                    choices: [
                        ...
                        {
                            name: "Generate WEI entities",
                            value: "weiEntities",
                            checked: options.generationOptions.weiEntities,
                        },
                    ],
        ...
        options.generationOptions.weiEntities =
            customizations.includes("weiEntities");
    ...
    if (options.generationOptions.weiEntities) {
        options.generationOptions.superClassName = (await inquirer.prompt([
            {
                default: options.generationOptions.superClassName,
                message: "WEI Model Superclass Name",
                name: "superClassName",
                type: "input",        
            },
        ])).superClassName;
            // estimate the depth to the base model
            if (options.generationOptions.resultsPath.includes("/models")) {
                
                const scName = options.generationOptions.superClassName;
                if (options.generationOptions.resultsPath.endsWith("/models") || options.generationOptions.resultsPath.endsWith("/models/")) {
                    options.generationOptions.superClassImportPath = `./${scName}`;
                } else {
                    const pathFromModels = options.generationOptions.resultsPath.split("/models")[1];
                    const depth = pathFromModels.split("/").length - 1;
                    if (depth === 0) {
                        options.generationOptions.superClassImportPath = `./${scName}`;
                    } else {
                        let npath = "";
                        ...
                        options.generationOptions.superClassImportPath = `${npath}${scName}`;
                    }
                }
            }
        
        options.generationOptions.superClassImportPath = (await inquirer.prompt([
            {
                default: options.generationOptions.superClassImportPath,
                message: "WEI Model Superclass Import Path?",
                name: "superClassImportPath",
                type: "input",        
            },
        ])).superClassImportPath;
    }
