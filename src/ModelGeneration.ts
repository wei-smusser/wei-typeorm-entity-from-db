...
export default function modelGenerationPhase(
    ...
    if (!generationOptions.noConfigs) {
        ...
        entitiesPath = (generationOptions.weiEntities) ?resultPath: path.resolve(resultPath, "./entities");
...
function generateModels(
    ...
    const entityTemplatePath = (generationOptions.weiEntities) ? 
    path.resolve(
        __dirname,
        "templates",
        "weientity.mst"
    ...
    fs.readFileSync(entityTemplatePath, "utf-8");
    ...
    databaseModel.forEach((element) => {
        ...
        if (generationOptions.weiEntities) {
            const fieldsToRemove = ["weiid","created_date","created_by","updated_date","updated_by","deactivated","w_srcid"];
            element.columns = element.columns.filter((column) => !fieldsToRemove.includes(column.tscName));
            element.relations = element.relations.filter((relation) => !fieldsToRemove.includes(relation.fieldName));
            element.relationIds = element.relationIds.filter((relationId) => !fieldsToRemove.includes(relationId.fieldName));
        }
        element.superClassName = generationOptions.superClassName;
        element.superClassImportPath = generationOptions.superClassImportPath;
        ...
        let formatted = "";
        try {
            console.log(withImportStatements);
